...
base_url = 'https://www.bigdata-forest.kr/product/'
...
dataset = pd.DataFrame({'sample_url': []})
k = 0
for j in range(1,24):
    k += 1
    body = browser.find_element_by_tag_name('body')  # 스크롤하기 위해 소스 추출
    while num_of_pagedowns:
        ...
        num_of_pagedowns -= 1
    for i in range(1,21):
        sample = browser.find_element_by_xpath('//*[@id="frmList"]/div/div[3]/div[2]/div/ul/li[' + str(i) + ']/a')
        url = sample.get_attribute('href')
        url = base_url + url[21:len(url)-2]
        sample_url.append(url)
    ...
    if j == 21:
        k-= 10
for i in range(0, len(sample_url)):
    insert_data = pd.DataFrame({
                                'sample_url': [sample_url[i]]})
